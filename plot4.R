...
par(mfrow=c(2,2))
with(df_subset, {
  plot(Time,Global_active_power,"l",ylab="Global Active Power")
  plot(Time,Voltage, "l", ylab="Voltate")
  {
    plot(Time,Sub_metering_1, "l", ylab="energy Sub metering", col="black") 
    lines(Time,Sub_metering_2,xlab="",col="red")
    lines(Time,Sub_metering_3,xlab="", col="blue")
    legend("topright",c("Sub_metering_1","Sub_metering_2","Sub_metering_3"), lty=c(1,1), lwd=c(2.5,2.5), 
             col= c("black", "red", "blue"))
  }
  plot(Time,Global_reactive_power,"l",ylab="Global_reactive_power")
})
